...
public class KundenHandler {
	...
	public String ausWarenkorblöschen() {
		...
		if (index >= merkeKunde.getArtikelDaten().size()) {
			...
			gesamtpreis = gesamtpreis - merkeKunde.getArtikelDaten().get(index).getPreis();
			gesamt = Math.floor(gesamt * 100) / 100.0;

			gesamtanzahl = gesamtanzahl - merkeKunde.getArtikelDaten().get(index).getAnzahl();
